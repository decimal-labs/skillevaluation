---
name: refund-policy
description: Use during a customer-support chat about a refund — applies a fictional "Acme Support" escalation policy (hold the no-refund line politely, escalate to a manager only after the customer has pushed back at least three times, and never admit company liability) so agents are consistent under pressure. Template: replace the policy with your own.
category: support
tags: [support, customer-service, policy, template]
stability: stable
---

> **Worked example (seeded-transcript policy testing, schema rev 2).** The policy below is a
> fictional "Acme Support" escalation policy, not DecimalAI's. Each eval case
